...
If Solidus was already installed with _solidus_frontend_ you will have to change
all `Spree::Frontend::Config` in `SolidusStarterFrontend::Config`.

You have the following 2 install options.

Run our executable to copy the component files in your project:
- installing it with: `gem install solidus_starter_frontend`;
- running in your project root folder: `solidus_starter_frontend`;
- customizing the local views;

Add our component as gem:
- adding to your _Gemfile_: `gem 'solidus_starter_frontend'` (**put this line 
before** `gem 'solidus_auth_devise'` **if you are using it**);
- overriding the views that needs customizations.
